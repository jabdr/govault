...
func newAdminClient() (*vault.Admin, error) {
	if server == "" {
		return nil, fmt.Errorf("server is required")
	}
	return vault.NewAdmin(server, adminToken, insecureSkipVerify, logger)
}
...
func adminUserCmd() *cli.Command {
	return &cli.Command{
		...
		Commands: []*cli.Command{
			...
			{
				Name:  "invite",
				Usage: "Invite a user by email",
				Flags: []cli.Flag{
					&cli.StringFlag{Name: "email", Required: true, Usage: "Email to invite"},
				},
				Action: func(ctx context.Context, cmd *cli.Command) error {
					adm, err := newAdminClient()
					if err != nil {
					...
					if err := adm.InviteUser(cmd.String("email")); err != nil {
						return err
					}
					printOutput(MessageResult{Message: fmt.Sprintf("Invited %s", cmd.String("email"))})
					return nil
				},
			},
			{
				Name:  "reinvite",
				Usage: "Re-send invitation email for a user",
				Flags: []cli.Flag{
					&cli.StringFlag{Name: "id", Required: true, Usage: "User ID"},
				},
				Action: func(ctx context.Context, cmd *cli.Command) error {
					adm, err := newAdminClient()
					if err != nil {
			...
			{
				Name:  "deauth",
				Usage: "Deauthorize all sessions for a user",
				Flags: []cli.Flag{
					&cli.StringFlag{Name: "id", Required: true, Usage: "User ID"},
				},
				Action: func(ctx context.Context, cmd *cli.Command) error {
			...
			{
				Name:  "remove-2fa",
				Usage: "Remove two-factor authentication for a user",
				Flags: []cli.Flag{
					&cli.StringFlag{Name: "id", Required: true, Usage: "User ID"},
				},
				Action: func(ctx context.Context, cmd *cli.Command) error {
		...
		Commands: []*cli.Command{
			{
				...
				Action: func(ctx context.Context, cmd *cli.Command) error {
					...
					for _, o := range orgs {
						results = append(results, AdminOrgResult{
							ID:           o.ID,
							Name:         o.Name,
